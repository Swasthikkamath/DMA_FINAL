...
interface apb_master_monitor_bfm (input bit pclk,
                                  ...
                                  input logic [apb_global_pkg::NO_OF_SLAVES-1:0]psel,
                                  input logic [apb_global_pkg::DATA_WIDTH-1:0]pwdata,
                                  input logic [(apb_global_pkg::DATA_WIDTH/8)-1:0]pstrb, 
  ...
  task sample_data (output apb_transfer_char_s apb_data_packet, input apb_transfer_cfg_s apb_cfg_packet);
    ...
    while(masterCb.psel != 1'b1 || masterCb.pready != 1'b1) begin
      @(masterCb);
      `uvm_info(name, $sformatf("Inside while loop PSEL"), UVM_DEBUG)
    end
   ...
   begin 
    ...
    if (pwrite == apb_global_pkg::WRITE) begin
    ...
    `uvm_info(name, $sformatf("APB_MASTER_MONITOR: SAMPLE_DATA=%p", apb_data_packet), UVM_HIGH)
  endtask : sample_data
