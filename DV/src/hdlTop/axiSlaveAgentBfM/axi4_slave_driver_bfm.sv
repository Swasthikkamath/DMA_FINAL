...
interface axi4_slave_driver_bfm(input                     aclk    , 
  ...
  task axi4_write_address_phase(inout axi4_write_transfer_char_s data_write_packet);
    ...
    `uvm_info("SLAVE_DRIVER_WADDR_PHASE", $sformatf("outside of awvalid"), UVM_DEBUG);
   ...
   // based on the wait_cycles we can choose to drive the awready
    `uvm_info(name,$sformatf("Before DRIVING WRITE ADDRS WAIT STATES :: %0d",data_write_packet.no_of_wait_states),UVM_DEBUG);
    repeat(data_write_packet.no_of_wait_states)begin
      `uvm_info(name,$sformatf("DRIVING_WRITE_ADDRS_WAIT_STATES :: %0d",data_write_packet.no_of_wait_states),UVM_DEBUG);
      @(axiSlaveCb);
  ...
  task axi4_write_data_phase (inout axi4_write_transfer_char_s data_write_packet, input axi4_transfer_cfg_s cfg_packet);
   ...
   `uvm_info("SLAVE_BFM_WDATA_PHASE",$sformatf("Before DRIVING WRITE DATA WAIT STATES :: %0d",data_write_packet.no_of_wait_states),UVM_DEBUG);
   repeat(data_write_packet.no_of_wait_states)begin
      `uvm_info(name,$sformatf("DRIVING_WRITE_DATA_WAIT_STATES :: %0d",data_write_packet.no_of_wait_states),UVM_DEBUG);
      @(axiSlaveCb);
      axiSlaveCb.wready<=0;
    end
    repeat(1)@(axiSlaveCb);
    ...
    forever begin
      ...
      axiSlaveCb.wready<=0;
      repeat(15)@(axiSlaveCb);
      axiSlaveCb.wready<=1;
   
      i++;
     end
         
    `uvm_info("slave_wlast",$sformatf("slave_wlast = %0b ,a=%0d",axiSlaveCb.wlast,a),UVM_DEBUG);
    `uvm_info("slave_wlast",$sformatf("sampled_slave_wlast = %0b",data_write_packet.wlast),UVM_DEBUG);
  ...
  task axi4_write_response_phase(inout axi4_write_transfer_char_s data_write_packet,
    ...
    while(axiSlaveCb.bready === 0) begin
      ...
      `uvm_info(name,$sformatf("inside_detect_bready = %0d",axiSlaveCb.bready),UVM_DEBUG)
    end
    `uvm_info(name,$sformatf("After_loop_of_Detecting_bready = %0d",axiSlaveCb.bready),UVM_DEBUG)
  ...
  task axi4_read_address_phase (inout axi4_read_transfer_char_s data_read_packet, input axi4_transfer_cfg_s cfg_packet);
    ...
    repeat(data_read_packet.no_of_wait_states)begin
      `uvm_info(name,$sformatf("DRIVING_READ_ADDRS_WAIT_STATES :: %0d",data_read_packet.no_of_wait_states),UVM_DEBUG);
      @(axiSlaveCb);
    ...
    `uvm_info("mem_arid",$sformatf("mem_arid[%0d]=%0d",j,mem_arid[j]),UVM_DEBUG)
    `uvm_info("mem_arid",$sformatf("arid=%0d",axiSlaveCb.arid),UVM_DEBUG)
    `uvm_info(name,$sformatf("struct_pkt_rd_addr_phase = \n %0p",data_read_packet),UVM_DEBUG)
