...
interface axi4_slave_monitor_bfm(input aclk, input aresetn,
 ...
 clocking slaveMonCb @(posedge aclk);
  ...
  task axi4_slave_write_address_sampling(output axi4_write_transfer_char_s req ,input axi4_transfer_cfg_s cfg);
    ...
    while(slaveMonCb.awvalid!==1 || slaveMonCb.awready!==1)begin
      @(slaveMonCb);
      `uvm_info("FROM SLAVE MON BFM",$sformatf("Inside while loop from axi4_slave_write_address_sampling"),UVM_DEBUG)
    end    
    ...
    `uvm_info("FROM SLAVE MON BFM",$sformatf("after while loop from axi4_slave_write_address_sampling req=%0p ",req),UVM_DEBUG)
  endtask
  ...
  task axi4_slave_write_data_sampling(output axi4_write_transfer_char_s req ,input axi4_transfer_cfg_s cfg);
   ...
   `uvm_info("FROM SLAVE MON BFM",$sformatf("Inside while loop......"),UVM_DEBUG)
    req.wdata[0] = slaveMonCb.wdata;
    req.wstrb[0] = slaveMonCb.wstrb;
    req.wlast =slaveMonCb.wlast;
    req.wuser[i] = slaveMonCb.wuser;

   `uvm_info("FROM SLAVE MON BFM write data",$sformatf("write datapacket wdata[%0d] = 'h%0x",i,req.wdata[i]),UVM_DEBUG)
   `uvm_info("FROM SLAVE MON BFM write data",$sformatf("write datapacket wstrb[%0d] = 'h%0x",i,req.wstrb[i]),UVM_DEBUG)
 ...
 endtask
  ...
  @(slaveMonCb);
    ...
    `uvm_info("FROM SLAVE MON BFM",$sformatf("values :: bvalid=%d & bready=%d",bvalid,bready),UVM_DEBUG)
      @(slaveMonCb);
      `uvm_info("FROM SLAVE MON BFM",$sformatf("Inside while loop of write response sample"),UVM_DEBUG)
    end    
    `uvm_info("FROM SLAVE MON BFM",$sformatf("after while loop of write response "),UVM_DEBUG)
  ...
  task axi4_read_address_sampling(output axi4_read_transfer_char_s req ,input axi4_transfer_cfg_s cfg);
    ...
    while(slaveMonCb.arvalid!==1 || slaveMonCb.arready!==1)begin
      @(slaveMonCb);
      `uvm_info("FROM SLAVE MON BFM READ ADDR",$sformatf("INSIDE WHILE LOOP OF READ ADDRESS"),UVM_DEBUG)
    end    
    `uvm_info("FROM SLAVE MON BFM READ ADDR",$sformatf("AFTER WHILE LOOP OF READ ADDRESS"),UVM_DEBUG)
    ...
    `uvm_info("FROM SLAVE MON BFM READ ADDR",$sformatf("datapacket =%p",req),UVM_DEBUG)
  endtask
  ...
  task axi4_read_data_sampling(output axi4_read_transfer_char_s req ,input axi4_transfer_cfg_s cfg);
    static reg[7:0] i = 0;
      ...
      `uvm_info("FROM SLAVE MON BFM READ DATA",$sformatf("DEBUG:SLAVE MON REQ.RID=%0d",req.rid),UVM_DEBUG)
      `uvm_info("FROM SLAVE MON BFM READ DATA",$sformatf("DEBUG:SLAVE MON RDATA[%0d]=%0h",i,rdata),UVM_DEBUG)
      `uvm_info("FROM SLAVE MON BFM READ DATA",$sformatf("DEBUG:SLAVE MON REQ.RDATA[%0d]=%0h",i,req.rdata[i]),UVM_DEBUG)
