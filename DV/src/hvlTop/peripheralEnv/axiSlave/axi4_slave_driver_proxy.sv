...
task axi4_slave_driver_proxy::axi4_write_task();
  ...
  forever begin :fb1
    ...
    begin : WRITE_ADDRESS_CHANNEL 
      ...
      `uvm_info("DEBUG_SLAVE_WRITE_ADDR_PROXY", $sformatf("AFTER :: Received req packet \n %s",local_slave_addr_tx.sprint()), UVM_DEBUG);
  ...
  //checking the status of write address thread
    addr_tx.await();
    `uvm_info("SLAVE_STATUS_CHECK",$sformatf("AFTER_FORK_JOIN_ANY:: SLAVE_ADDRESS_CHANNEL_STATUS =\n %s",addr_tx.status()),UVM_DEBUG)
    `uvm_info("SLAVE_STATUS_CHECK",$sformatf("AFTER_FORK_JOIN_ANY:: SLAVE_WDATA_CHANNEL_STATUS = \n %s",data_tx.status()),UVM_DEBUG)
    `uvm_info("SLAVE_STATUS_CHECK",$sformatf("AFTER_FORK_JOIN_ANY:: SLAVE_WRESP_CHANNEL_STATUS = \n%s",response_tx.status()),UVM_DEBUG)
...
task axi4_slave_driver_proxy::axi4_read_task();
  ...
  forever begin 
    ...
    begin : READ_ADDRESS_CHANNEL
      ...
      waitStates++; 
      `uvm_info("DEBUG_SLAVE_READ_ADDR_PROXY", $sformatf("AFTER :: Received req packet \n %s",local_slave_tx.sprint()), UVM_DEBUG);
 ...
 //     axi4_read_transfer_char_s  struct_read_packet;
      ...
      `uvm_info("DEBUG_SLAVE_RDATA_PROXY", $sformatf("AFTER :: READ CHANNEL PACKET \n %s",local_slave_rdata_tx.sprint()), UVM_DEBUG);
    ...
    rd_addr.await();
    `uvm_info("SLAVE_STATUS_CHECK",$sformatf("AFTER_FORK_JOIN_ANY:: SLAVE_READ_CHANNEL_STATUS = \n %s",rd_addr.status()),UVM_DEBUG)
    `uvm_info("SLAVE_STATUS_CHECK",$sformatf("AFTER_FORK_JOIN_ANY:: SLAVE_RDATA_CHANNEL_STATUS = \n %s",rd_data.status()),UVM_DEBUG)
...
task axi4_slave_driver_proxy::task_memory_write(input axi4_slave_tx struct_write_packet);
 ...
 if(struct_write_packet.awburst == WRITE_FIXED) begin
    for(int j=0;j<(struct_write_packet.awlen+1);j++)begin
      `uvm_info("DEBUG_MEMORY_WRITE",$sformatf("memory_task_awlen=%d",struct_write_packet.awlen),UVM_DEBUG)
        for(int strb=0;strb<STROBE_WIDTH;strb++) begin
        `uvm_info("DEBUG_MEMORY_WRITE", $sformatf("task_memory_write inside for loop wstrb = %0h",struct_write_packet.wstrb[strb]), UVM_DEBUG);
  ...
  if(struct_write_packet.awburst == WRITE_INCR) begin
    ...
    int amount;
       if(addr % (2**struct_write_packet.awsize) != 0) begin
             ...
             unalign=1;      
         `uvm_info("SLAVE DRIVER PROXY",$sformatf("THE WRITE ADDRESS IS UNALIGNED BY AN AMOUNT %0d WHEN THE ARSIZE IS %0d AND ADDRESS IS %0d",amount,struct_write_packet.awsize,addr),UVM_DEBUG)
       end
    for(int j=0,int k=0;j<(struct_write_packet.awlen+1);j++)begin
      `uvm_info("DEBUG_MEMORY_WRITE",$sformatf("memory_task_awlen=%d",struct_write_packet.awlen),UVM_DEBUG)
        ...
        for(int strb=0,k=0;strb<((2**struct_write_packet.awsize)-amount);strb++) begin
           `uvm_info("DEBUG_MEMORY_WRITE", $sformatf("task_memory_write inside for loop wstrb = %0h,k=%0d",struct_write_packet.wstrb[strb],k), UVM_DEBUG);
          k = addr % (DATA_WIDTH/8);
          if(struct_write_packet.wstrb[j][k] == 1) begin
            `uvm_info("SLAVE DRIVER PROXY",$sformatf("THE BYTE WRITTEN TO THE MEMORY IS %h AND THE ADDRESS IS %0d",struct_write_packet.wdata[j][8*k+7 -: 8],addr),UVM_DEBUG)
  ...
  if(struct_write_packet.awburst == WRITE_WRAP) begin
   ...
   `uvm_info("SLAVE DRIVER PROXY",$sformatf("WRITE WRAP TRANSFER ADDRESS BOUNDARY IS %0d to %0d",lower_addr,end_addr),UVM_DEBUG)
...
struct_read_packet.rid = read_pkt.arid;
  ...
  if(read_pkt.arburst == READ_INCR) begin
    ...
    int amount;
       if(read_pkt.araddr % (2**read_pkt.arsize) != 0) begin
             ...
             unalign=1;
            `uvm_info("SLAVE DRIVER PROXY",$sformatf("THE READ ADDRESS IS UNALIGNED BY AN AMOUNT %0d WHEN THE ARSIZE IS %0d AND ADDRESS IS %0d",amount,read_pkt.arsize,addr),UVM_DEBUG)
  ...
  if(read_pkt.arburst == READ_WRAP) begin
    ...
    `uvm_info("SLAVE DRIVER PROXY",$sformatf("READ WRAP TRANSFER ADDRESS BOUNDARY IS %0d to %0d",lower_addr,end_addr),UVM_DEBUG)

    k_t = read_pkt.araddr;
       if(k_t % (2**read_pkt.arsize) != 0) begin
             amount = k_t - ((k_t/(2**read_pkt.arsize))*(2**read_pkt.arsize));
             unalign=1;
            `uvm_info("SLAVE DRIVER PROXY",$sformatf("THE READ ADDRESS IS UNALIGNED BY AN AMOUNT %0d WHEN THE ARSIZE IS %0d AND ADDRESS IS %0d",amount,read_pkt.arsize,k_t),UVM_DEBUG)
        ...
        for(int strb=0;strb<((2**read_pkt.arsize)-amount);strb++) begin
          if(k_t < end_addr)  begin
           ...
           if(axi4_slave_mem_h.is_slave_addr_exists(k_t))begin  
             ...
             struct_read_packet.rresp[0] = READ_OKAY;
             `uvm_info("SLAVE DRIVER PROXY",$sformatf("SLAVE WRAP READ THE DATA EXISTS READ FROM %0d AND READ DATA IS %0d",k_t,struct_read_packet.rdata[0][8*k+7 -: 8]),UVM_DEBUG)
             k_t++;
           end 
           else begin
            `uvm_info("SLAVE DRIVER PROXY",$sformatf("SLAVE WRAP READ THE DATA DOESNT EXIST READ FROM %0d",k_t),UVM_DEBUG)
...
task axi4_slave_driver_proxy::out_of_order_for_reads(output axi4_read_transfer_char_s oor_read_data_struct_read_packet);
 ...
 `uvm_info("slave_driver_proxy",$sformatf("fifo_size = %0d",axi4_slave_read_addr_fifo_h.used()),UVM_DEBUG)
